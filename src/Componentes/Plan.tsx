...
const Plan = () => {
  ...
  return (
    ...
    <div className=""> 
    <div className="pt-20 pb-20 bg-gradient-to-b dark:from-slate-800 dark:via-transparent dark:to-transparent from-gray-200 via-transparent to-transparent">
      <div className="container mx-auto max-w ">
        <div className="mb-20">
          <h2 className="text-3xl md:text-4xl font-medium text-center">Choose your Plan</h2>
          <p className="dark:text-slate-400 text-lg md:text-xl mt-4 md:mt-8 text-center text-[#4F5665]">Let's choose the package that is best for you and explore it happily and cheerfully.</p>
        </div>
        <div className="flex flex-col md:flex-row justify-center md:space-x-16 space-y-8 md:space-y-0 w-full">
          {sub.map((subscription) => (
            <div className=" flex flex-col p-6 md:p-16 dark:bg-slate-800 bg-white shadow-md items-center rounded-lg" key={subscription.title}>
              ...
              <div className="mb-6">
                {subscription.benefits.map((benefit) => (
                  <div className="flex items-baseline mb-2">
                    ...
                    <p className="dark:text-slate-400 ml-3 text-sm mt-2 text-[#4F5665]">{benefit}</p>
                  </div>
                ))}
              </div>

              <div className="mt-auto">
                <p className="mb-4 text-center text-xl font-medium">{subscription.currency && `${subscription.currency}`}
                  {typeof subscription.price === 'number'
                    ? `${subscription.price} /mo`
                    : subscription.price}</p>
                <button className="text-base button-color-basic font-semibold border border-[#F53838] px-4 py-2 rounded-full cursor-pointer pl-10 pr-10 boton-fondo">Select</button>
              </div>
